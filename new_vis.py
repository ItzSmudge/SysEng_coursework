...
def visualize_trajectory_interactive(pendulum, initial_state, controller, steps=10000, target_pos=[0.0, 0.0], save_gif=False):
    ...
    def reset_clicked(event):
        """Reset to t=0, pause, and update display"""
        anim_state['paused'] = True
        anim_state['frame'] = 0
        trace_x.clear()
        trace_y.clear()
    ...
    def animate(i):
        ...
        disturbance_memory[-1] = sim_data['disturbance_log'][idx] if sim_data['disturbance_log'] is not None else 0

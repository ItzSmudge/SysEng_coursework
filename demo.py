...
if __name__ == "__main__":
    ...
    # ============================================================
    # SHARED PHYSICAL PARAMETERS  (measured from real hardware)
    # ============================================================
    M   = 1.3816   # cart mass (kg)
    m   = 0.05     # pendulum bob mass (kg)
    l   = 0.6      # rod length to bob centre of mass (m)
    b   = 0.1      # friction coefficient (estimated)

    ANGLE_0   = np.array([0.0,    0.0, 0.0,              0.0])  # upright, balanced
    ANGLE_6   = np.array([0.0,    0.0, np.radians(6.0),  0.0])  # 6 degree initial tilt

    # LQR Q/R tuned to produce K ≈ [0, 0, 700, 2250] (validated on hardware)
    Q_lqr = np.diag([1.0, 1.0, 100.0, 10.0])
    R_lqr = np.array([[0.1]])

    # PID gains (tuned on hardware)
    PID_BALANCE = dict(kp_theta=120.0, kd_theta=20.0, ki_theta=5.0,
                       kp_x=0.0,       kd_x=0.0,      ki_x=0.0)
    PID_SPRINT  = dict(kp_theta=120.0, kd_theta=20.0, ki_theta=5.0,
                       kp_x=5.0,       kd_x=10.0,     ki_x=0.1)

    # ============================================================
    # SCENARIO 1 — PID: balance from upright
    # ============================================================
    # initial_state = ANGLE_0.copy()
    # controller = PIDController(**PID_BALANCE, dt=0.001, filter_enabled=True, window_size=50)
    ...
    # ============================================================
    # SCENARIO 2 — LQR: balance from upright
    # ============================================================
    # initial_state = ANGLE_0.copy()
    ...
    # ============================================================
    # SCENARIO 3 — PID: start at 6 degrees, balance to upright
    # ============================================================
    # initial_state = ANGLE_6.copy()
    # controller = PIDController(**PID_BALANCE, dt=0.001, filter_enabled=True, window_size=50)
    # pend = Pendulum(M=M, m=m, l=l, b=b, dt=0.001, mode="1", disturbance_level=0)
    # visualize_trajectory_interactive(pend, initial_state, controller, steps=10000, target_pos=[0.0, 0.0])

    # ============================================================
    # SCENARIO 4 — LQR: start at 6 degrees, balance to upright
    # ============================================================
    # initial_state = ANGLE_6.copy()
    # controller = LQRController(M=M, m=m, l=l, b=b, Q=Q_lqr, R=R_lqr, filter_enabled=True, window_size=50)
    # pend = Pendulum(M=M, m=m, l=l, b=b, dt=0.001, mode="1", disturbance_level=0)

...
// Serial PWM Output Version (No Motor Control)
// ============================================
...
void setup() {
  ...
  Serial.println("========================================");
  Serial.println("PID CONTROLLER WITH MOVING AVERAGE FILTER");
  Serial.println("Serial PWM Output Version");
  Serial.println("========================================");
...
void loop() {
  ...
  if (currentTime - lastControl >= 10) {
    ...
    // Convert force to PWM value (0-255 range)
    int pwmValue = forceToPWM(force);

    // Output PWM to serial
    Serial.print("PWM: ");
...
int forceToPWM(float force) {
  // Convert force to PWM value (-255 to 255)
  int pwmValue = (int)constrain(force, -255, 255);
  return pwmValue;
}
